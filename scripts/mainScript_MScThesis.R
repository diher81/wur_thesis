...
aS.eQTL.table <- aS.eQTL.table %>%
  dplyr::filter(gene_chromosome == "III",
                dplyr::between(gene_bp, 1000000, 2500000))
...
for (i in seq_along(spotIds)) {
  ...
  plotEqtlProfile <- ggplot(
    ...
    aes(x = qtl_bp, y = qtl_significance, alpha = 0.2)) +
    geom_line(
      size = 1.5,
  ...
  plotBoxplot <- ggplot(
    data.plot[[2]],
    aes(x = geno_strain, y = trait_value)) +
    geom_jitter(
      height = 0,
      width = 0.25,
    ...
    annotate(
      "text",
      x = 1.5,
      y = max(data.plot[[2]]$trait_value, na.rm = TRUE),
      label = paste0(
        "italic(R)^{2}==",
