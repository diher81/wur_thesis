...
transcriptomics.check.genes <- function(trans.int,spot.id,genes.to.check,filename,save_dir){
                                        if(missing(spot.id)){                                   stop("Require spot IDs (gene names)")}
                                        if(missing(genes.to.check)){                            genes.to.check <- c("gst-27","vit-1","vit-2","hsp-16.2","hsp-16.41")}
