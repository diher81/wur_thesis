...
QTL.map1.dataframe <- function(map1.output, small) {
  ...
  message("[QTL] Checking input structure...")
  if (is.null(dim(map1.output[[1]]))) {
    small <- TRUE
  }
  
  if (small) {
    message("[QTL] Allocating output matrix (small)...")
    ###Split up to long format
    output <- matrix(NA, length(map1.output[[1]]), 6)
    
    message("[QTL] Filling columns...")
    output[, 1] <- rep(rownames(map1.output[[3]])[1], each =
                         length(map1.output[[1]]))
  ...
  if (!small) {
    message("[QTL] Allocating output matrix (full)...")
    ###Split up to long format
    output <- matrix(NA, ncol(map1.output[[1]]) *
                       nrow(map1.output[[1]]), 6)
    
    message("[QTL] Filling columns...")
    output[, 1] <- rep(rownames(map1.output[[1]]), each =
                         ncol(map1.output[[1]]))
    output[, 2] <- rep(as.character(unlist(map1.output[[5]][, 2])), times =
                         nrow(map1.output[[1]]))
    output[, 3] <- rep(as.numeric(as.character(unlist(map1.output[[5]][, 3]))), times =
                         nrow(map1.output[[1]]))
    output[, 4] <- rep(c(1:ncol(map1.output[[1]])), times =
                         nrow(map1.output[[1]]))
    message("[QTL] Flattening matrices (this may take time)...")
    output[, 5] <- c(t(map1.output[[1]]))
    output[, 6] <- c(t(map1.output[[2]]))
    message("[QTL] Converting to data.frame...")
    output <- as.data.frame(output)
  }
  
  ###overwrite dataframe formats
  message("[QTL] Fixing column types...")
  ...
  end_time <- Sys.time()
  message(sprintf("[QTL] Completed in %.2f seconds",
